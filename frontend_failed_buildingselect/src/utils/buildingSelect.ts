...
export class BuildingSelector {
  ...
  _extrusionLayerIds() {
    if (this.layerIds) return this.layerIds.filter((id: any) => this.map.getLayer(id));
    return this.map.getStyle().layers
      .filter((l: any) => l.type === 'fill-extrusion' && l.id !== HL_LAYER)
      .map((l: any) => l.id);
  }
  ...
  _overlapsEnough(a: any, b: any, areaA: number, areaB: number) {
  ...
  _collectBuilding(seed: any, zoom: number) {
    ...
    if (expand) {
      ...
      let remaining = candidates.filter((f: any) => !(hasIds && selectedIds.has(f.id)));
      ...
      while (frontier.length && selectedFeatures.length < this.maxFeatures) {
        ...
        for (const f of frontier) {
          ...
          remaining = remaining.filter((c: any) => {
  ...
  _renderOverlay(sel: any) {
    ...
    const existing: any = this.map.getLayer(HL_LAYER);

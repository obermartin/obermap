...
import { BuildingSelector } from '../utils/buildingSelect';
  ...
  useEffect(() => {
    ...
    const onClick = (e: maplibregl.MapMouseEvent) => {
      ...
      if (activeTool === 'highlight') {
        ...
        const bsel = new BuildingSelector(map);
        const extrusionLayers = bsel._extrusionLayerIds();
        let buildingHits: maplibregl.MapGeoJSONFeature[] = [];
        if (extrusionLayers.length > 0) {
           buildingHits = map.queryRenderedFeatures(e.point, { layers: extrusionLayers });
        }
        if (buildingHits.length > 0) {
           const seed = buildingHits[0];
           const sel = bsel._collectBuilding(seed, map.getZoom());
           bsel.destroy();
           
           const feats: any[] = [];
           for (const f of sel.features) {
             let g;
             try { g = turf.buffer(f, bsel.overlayGrowM, { units: 'meters' }); } catch { continue; }
             if (!g) continue;
             g.properties = {
               h: bsel._height(f) + 1,
               b: bsel._minHeight(f),
           ...
           setAnnotations(prev => [...prev, {
             ...
             theme: actualTheme,
             coordinates: [e.lngLat.lng, e.lngLat.lat],
             polygonGeometry: { type: 'FeatureCollection', features: feats },
             is3DBuilding: true,
             ...
             view: {
               ...
               elevation: mapRef.current!.queryTerrainElevation([e.lngLat.lng, e.lngLat.lat]) || 0
             }
           }]);
           return;
        }

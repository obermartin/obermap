...
interface FlightDataProps {
  ...
  updateDeckGL: (dataStates: any[]) => void;
  t: (key: string) => string;
}
...
export const useFlightData = ({
  ...
  const latestDataStatesRef = useRef<any[]>([]);

  // Polling for flights
  useEffect(() => {
    if (!map || !mapLoaded || !flightsLayer || !isFlightsVisible) return;
    ...
    const fetchFlights = async () => {
      ...
      try {
        ...
        latestDataStatesRef.current = data.states || [];
            ...
            if (flightsLayer.is3DMode) {
              ...
              if (!aircraftPopupRef.current) {
                import('maplibre-gl').then((mgl) => {
                    aircraftPopupRef.current = new mgl.default.Popup({ closeButton: false, closeOnClick: false, className: 'flight-popup' })
                    .setLngLat([lon, lat])
                    .addTo(map);
                });
              } else {
              ...
              if (aircraftPopupRef.current) aircraftPopupRef.current.setHTML(popupHtml);
            }
          }
    ...
    const onZoom = () => {
      if (latestDataStatesRef.current) {
        updateDeckGL(latestDataStatesRef.current);
      }
    };
    map.on('zoom', onZoom);
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('exportDataReady', {
      detail: { type: 'flights', id: selectedAircraftIdRef.current, ready: false }
    }));
    ...
    const fetchTrack = async () => {
      try {
        ...
        const url = `./api.php?action=opensky_track&icao24=${selectedAircraftIdRef.current}&time=0${token ? '&token=' + encodeURIComponent(token) : ''}`;
        const res = await fetch(url);
        ...
        if (data && data.path && data.path.length > 0) {
          ...
          const hist = flightHistoryRef.current[selectedAircraftIdRef.current || ''];
          ...
          window.dispatchEvent(new CustomEvent('exportDataReady', {
            detail: { type: 'flights', id: selectedAircraftIdRef.current, ready: coordinates.length > 0 }
          }));
          source.setData({
            type: 'FeatureCollection',
            features: [{
          ...
          selectedFlightTrackRef.current = [];
          window.dispatchEvent(new CustomEvent('exportDataReady', {
            detail: { type: 'flights', id: selectedAircraftIdRef.current, ready: 'empty' }
          }));
          source.setData({ type: 'FeatureCollection', features: [] });
        }
      } catch (err) {
      ...
      try {
        ...
        const metaRes = await fetch(`./api.php?action=opensky_metadata&icao24=${selectedAircraftIdRef.current}${token ? '&token=' + encodeURIComponent(token) : ''}`);
        let metaData: any = null;
        if (metaRes.ok) metaData = await metaRes.json();
        
        selectedAircraftMetaRef.current = {
          ...selectedAircraftMetaRef.current,
          icao24: selectedAircraftIdRef.current,
    ...
    fetchTrack();
  }, [selectedAircraftIdRef.current, mapLoaded, settings.openSkyCredentials]);
};
